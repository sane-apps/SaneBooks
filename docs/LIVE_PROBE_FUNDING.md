# How to get a UFVK with history (SaneBooks live proof)

You do **not** need to invent a key. Use the built-in live probe account, then put **any** small ZEC amount on it.

## Fast path (recommended)

1. In SaneBooks: **Import Viewing Key → Use Live Probe Key → Import**.
...
4. In SaneBooks: sync again (or reopen the vault). You should see ≥1 inbound note.
...
Export that wallet’s **UFVK** (`uview…`) instead — see `WALLET_VIEWING_KEY_GUIDE.md` — and import it into SaneBooks with the wallet birthday. That is better for “your books”; the probe path is only for grant/dev proof.
