# Competitive positioning — ZecBooks

One page. Audience: coinholders reviewing a Retroactive Grant draft.

**ZecBooks lane:** local Mac, view-only, **owner → CPA selective disclosure** (classify + scoped, expiring proof pack).  
...
| Product | What it is | Key / privilege | Where ZecBooks differs |
|---------|------------|-----------------|-------------------------|
| **CipherPay** | Non-custodial merchant payments: IVK trial decrypt, hosted/self-host checkout, CSV | **UIVK / IVK** receivables | CipherPay is **before/at** payment. ZecBooks is **after** money arrives. |
| **ZBooks** ([AustinChris1/ZBooks-SIWZ](https://github.com/AustinChris1/ZBooks-SIWZ)) | Team/DAO treasury accounting + SIWZ auth + M-of-N approvals + ZIP 321 batch payouts; web + Turso | UFVK for treasury read; payouts signed in treasurer wallet | **Different job.** ZBooks = multi-user treasury ops and money-out. ZecBooks = solo owner → CPA on a **local Mac**; offline packs + Reader; **no** payout URI, no SIWZ, no hosted ledger DB. |
| **Zodl** | Modern spend wallet; UFVK / `data.db` export | Seed + spend | Wallet. ZecBooks imports UFVK or Zashi/Zodl `data.db` only. |
| **ZGo** | Historical merchant / order tooling | Merchant confirm | Order/checkout era. ZecBooks is ledger + CPA pack. |
| **Koinly** (and similar) | Multi-chain tax SaaS; weak on shielded ZEC | Exchange/CSV | Broad tax automation. ZecBooks is Zcash-native shielded classification. |
...
| Private Mac books + scoped CPA pack | **ZecBooks** |
...
Rejected applications show that novelty, a low ask, research quality, a demo, or claimed user counts are not sufficient alone. Voter motives are not fully published, so ZecBooks must not claim to know why any one application lost.
...
- Keep license language precise: ZecBooks is **MIT** open source. `CONTRIBUTING.md` is published.
