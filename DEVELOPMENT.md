...
| Release/signing/updates | BLOCKED. Project wrapper fails closed with exit 78, but the shared `release.sh` still does not enforce `release.enabled: false`; no public release authorized | fix shared gate outside this repo; version bump; sandboxed Release; exact entitlements/signature/Gatekeeper/notary/staple/download/update/upgrade |
| Website/privacy/support/ops | BLOCKED, no public SaneBooks site/download/support E2E | truthful privacy/help/download/release notes; private support path; synthetic delivery/read-back |
| Competition/grant | OPEN and separate from release | notarized public artifact, exact budget/metrics, and written source-available eligibility guidance |
...
10. Exact sandboxed Release → clean install/quarantine → real representative history → proof-pack round trip → provenance receipt.
