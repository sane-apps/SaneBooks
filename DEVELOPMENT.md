...
| Release/signing/updates | PASS GitHub DMG lane A: `release.enabled` fail-closed in shared `release.sh`; Developer ID Release archive; notarized+stapled `SaneBooks-0.1.0.zip`; Gatekeeper Notarized Developer ID; public https://github.com/sane-apps/SaneBooks/releases/tag/v0.1.0 (Sparkle/App Store remain off) | Sparkle/appcast/`sanebooks.com`; Mac App Store; upgrade-from-prior-public when a second build exists |
| Website/privacy/support/ops | BLOCKED, no public SaneBooks site/download/support E2E (GitHub Release is the download surface for lane A) | truthful privacy/help/download/release notes; private support path; synthetic delivery/read-back |
